...
/**
 * Tests pour les notifications Craft RPG
 */
class CraftRPGNotificationsTest : BasePlatformTestCase() {
    ...
    fun `test notifyLevelUp should create important notification`() {
        CraftRPGNotifications.notifyLevelUp(
            project = project,
            oldLevel = 1,
            newLevel = 2,
    ...
    fun `test notifyCombo should create notification with multiplier`() {
        CraftRPGNotifications.notifyCombo(
            project = project,
    ...
    fun `test notifyAchievement should create important notification`() {
        CraftRPGNotifications.notifyAchievement(
            project = project,
    ...
    fun `test multiple notifications should all be created`() {
        ...
        CraftRPGNotifications.notifyXPGain(project, action1)
        CraftRPGNotifications.notifyXPGain(project, action2)
    ...
    fun `test different action types should have different gameplay tags`() {
        ...
        CraftRPGNotifications.notifyXPGain(project, renameAction)

        val extractAction = RefactoringAction(type = RefactoringActionType.EXTRACT_METHOD)
        CraftRPGNotifications.notifyXPGain(project, extractAction)
