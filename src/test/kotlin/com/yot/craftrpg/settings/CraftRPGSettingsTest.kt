...
/**
 * Tests pour les paramètres Craft RPG
 */
class CraftRPGSettingsTest : BasePlatformTestCase() {

    private lateinit var settings: CraftRPGSettings

    override fun setUp() {
        super.setUp()
        settings = CraftRPGSettings.getInstance(project)
    }
    ...
    fun `test getInstance should return same instance`() {
        val settings1 = CraftRPGSettings.getInstance(project)
        val settings2 = CraftRPGSettings.getInstance(project)
    ...
    fun `test state should be persisted`() {
        ...
        state shouldBe CraftRPGSettings.State(
    ...
    fun `test should load state correctly`() {
        val newState = CraftRPGSettings.State(
            showXPNotifications = false,
